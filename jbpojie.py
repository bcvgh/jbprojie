...
c_ver = conf.get("Version","clion")
go_ver = conf.get("Version","goland")
...
if __name__ == '__main__':
    ...
    if par == 'python':
        ...
        delfile(parm + '\\' + c_ver+'\\eval')
        delfile(parm + '\\' + c_ver+'\\options\other.xml')
        delkey(par)
        print('clion试用期重置成功！')
    elif par == 'go':
        delfile(parm + '\\' + go_ver+'\\eval')
        delfile(parm + '\\' + go_ver+'\\options\other.xml')
        delkey(par)
        print('goland试用期重置成功！')
    else:
        print('unknow')